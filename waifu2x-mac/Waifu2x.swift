...
public class Waifu2x {
    ...
    static public func run(_ image: NSImage!, model: Model!, _ callback: @escaping (String) -> Void = { _ in }) -> NSImage? {
        ...
        let originCGImage = image.cgImage(forProposedRect: nil, context: nil, hints: nil)
        var hasalpha = originCGImage?.alphaInfo != CGImageAlphaInfo.none
        ...
        let outImage = NSImage.init(cgImage: cgImage, size: NSSize.init(width: out_width, height: out_height))
        callback("finished")
        return outImage
    }
